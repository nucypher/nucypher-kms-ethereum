...
class Miner(Actor):
    ...
    def fetch_miner_ids(self) -> tuple:
        ...
        count = self.miner_agent.call().getMinerIdsCount(self.address)

        miner_ids = list()
        for index in range(count):
            miner_id = self.miner_agent.call().getMinerId(self.address, index)
    ...
    def withdraw(self, amount: int=0, entire_balance=False) -> str:
        ...
        if entire_balance and amount:
            ...
            txhash = self.miner_agent.transact({'from': self.address}).withdrawAll()
        else:
